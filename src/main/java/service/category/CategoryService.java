...
public class CategoryService implements ICategoryService{
    ...
    public List<Category> findAll() throws SQLException {
        // Tạo một danh sách để lưu trữ các đối tượng Category
        List<Category> categoryList = new ArrayList<>();

        // Thực hiện truy vấn đến cơ sở dữ liệu
        ConnectionJDBC Database = null;
        try (Connection conn = Database.getConnection();
             PreparedStatement statement = conn.prepareStatement("SELECT * FROM categories");
             ResultSet resultSet = statement.executeQuery()) {

            // Lặp qua các kết quả từ truy vấn và thêm chúng vào danh sách
            while (resultSet.next()) {
                // Đọc thông tin của mỗi danh mục từ kết quả truy vấn
